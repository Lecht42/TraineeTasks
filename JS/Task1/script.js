...
let allData = [testData, testData2, testData3, testData4];


function cloneDeep (original){
	return JSON.parse(JSON.stringify(original)); // Даты, бесконечности и undefined не используются во входных данных, так что нам этот способ подойдёт
}

allData.forEach(data => {
	let copy = cloneDeep(data);
	copy.unshift({"copy":"yes"}); // Проверяем вдруг это всего лишь ссылка на объект testData, если это так то и в оригинале изменится содержимое, а именно добавится поле 'copy' со значением 'yes'
	console.log("Оригинал " + JSON.stringify(data));
